...
/opt/keycloak/bin/kcadm.sh update events/config -r master -s 'eventsListeners=["jboss-logging","openfga-events-publisher"]'
...
# Clients
/opt/keycloak/bin/kcadm.sh create clients -r master -s clientId=portal -s publicClient=true -s 'redirectUris=["http://localhost:9090/callback"]' -s 'webOrigins=["http://localhost:9090"]' -s 'attributes={ "post.logout.redirect.uris": "http://localhost:9090/home?action=logout", "access.token.lifespan": 3600}' -o
